...
- [ ] Запустить TaskService: `./gradlew bootRun` в папке TaskService
- [ ] Проверить создание задачи с любым String статусом (через POST /api/tasks)
- [ ] Проверить обновление статуса на произвольное значение (например "PROGRESS")
- [ ] Проверить получение задач со статусом (GET /api/tasks) — статус должен быть String
...
- [ ] Проверить что старые enum значения все еще работают (например "IN_PROGRESS")
- [ ] Проверить dashboard endpoint — подсчеты по статусам должны работать

...
### Task 8: Компиляция и базовая проверка

- [x] откомпилировать OrganizationService: `./gradlew compileKotlin compileJava`
- [x] убедиться что нет ошибок компиляции основного кода
- [x] EmployeesMapper исправлена с явными методами преобразования

### Task 9: Ручное тестирование создания организации

**Manual Testing:**
- [ ] запустить OrganizationService: `cd OrganizationService && ./gradlew bootRun`
- [ ] убедиться что сервис стартует без ошибок
- [ ] создать организацию через POST /organizations с корректным userId (используя JWT token)
- [ ] проверить в БД что:
...
- [x] добавить примечание что при создании организации автоматически создаётся владелец

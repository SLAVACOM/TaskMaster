...
//		mutableHeaders.putAll(exchange.getRequest().getHeaders());
		...
		if (StringUtils.hasText(role)) {
			mutableHeaders.add("X-User-Role", role);
			log.debug("RequestHeaderFilter: Added header X-User-Role={}", role);
		}
		if (StringUtils.hasText(profileId)) {
			mutableHeaders.add("X-Profile-Id", profileId);
			log.debug("RequestHeaderFilter: Added header X-Profile-Id={}", profileId);
		}
		if (StringUtils.hasText(organizationId)) {
			mutableHeaders.add("X-Organization-Id", organizationId);
			log.debug("RequestHeaderFilter: Added header X-Organization-Id={}", organizationId);
		}

		// Create new request with mutable headers using the wrapper
		ServerHttpRequest newRequest = new MutableHttpServerRequest(
				exchange.getRequest(),
				mutableHeaders
		);
